...
public class JwtIntercepter implements HandlerInterceptor {
	...
	if (!req.getMethod().equals("OPTIONS")) {// API Request(preFlight -> response -> Flight)인 경우 preFlight인 경우 제외
		...
		String jwtToken = req.getRequestURI().split("\\/")[1].equals("View")
			? req.getParameter("JWTForPostluck")
			: req.getHeader("JWTForPostluck");

		if (jwtToken != null && jwtToken.length() > 0) {
		    if ((result = this.jwtService.tokenExpiredDateCheck(jwtToken, userKey)) == true) {
			result = true;
		    } else
			message = "error:Session 오류:세션이 만료되었습니다.다시 로그인 해주세요.(jwt만료):";
		} else
		    message = "error:Session 오류:세션이 만료되었습니다.다시 로그인 해주세요.(jwt null):";
	    } else
		message = "error:접근 오류:비정상적인 접근 경로입니다.(get으로 접근):";
	} else
	    result = true;
	if(message!=null) {	    
    ...
    /*
     * Spring Framework 6.0 지원 현재 Spring Boot 에서 지원 >> postHandle, afterCompletion
     * 실행 과정 없음 public void afterConcurrentHandlingStarted(HttpServletRequest req,
